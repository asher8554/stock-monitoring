...
- 사용자는 자산비중을 막대 목록보다 도넛형 그래프로 보는 편이 더 좋았다고 했다.
- 새 의존성을 추가하지 않고 이미 프로젝트에 있는 Recharts `PieChart`를 `LegacyPortfolioView`에 다시 사용한다.
- 자산비중 범례와 금액은 유지하고, 도넛 중앙에는 총 잔고를 표시한다.
- `npm test`는 13개 파일 41개 테스트가 통과했고, `npm run build`도 통과했다.
- 브라우저 QA에서 desktop과 390px mobile 모두 도넛형 그래프와 범례가 보이고 console error/warn이 0개임을 확인했다.
...
- 같은 차트에 리먼브라더스와 코로나19 같은 위기 시점을 점선으로 표시하길 원했다.
